...
function renderReceiptPreview() {
    ...
    const totalQty = lastOrder.items.reduce((s, i) => s + i.qty, 0);
    ...
    const taglineHtml = (kasirEmp && kasirEmp.address)
        ? `<div class="rc-tagline">${kasirEmp.address}</div>${kasirEmp.phone ? `<div class="rc-tagline">Hp. ${kasirEmp.phone}</div>` : ''}`
        : `<div class="rc-tagline">Digital Point of Sales</div>`;

    const itemsHtml = lastOrder.items.map((i, idx) => {
        const variantLines = i.variantSelections
            ? i.variantSelections.map(v => `<div class="rc-row" style="padding-left:10px;color:#64748b;font-size:10px;"><span>- ${v.name} x${v.qty}</span></div>`).join('')
            : '';
        return `
            <div class="rc-item-name">${idx + 1}. ${i.name}</div>
            <div class="rc-row">
    ...
    el.innerHTML = `
        ...
        <div class="rc-row"><span>${lastOrder.date}</span></div>
        <div class="rc-row"><span>No. ${lastOrder.id}</span><span>${lastOrder.employeeName || '-'}</span></div>
        <div class="rc-dashed"></div>
        ${itemsHtml}
        <div class="rc-dashed"></div>
        <div class="rc-row"><span>Total QTY</span><span>${totalQty}</span></div>
        <div class="rc-row"><span>Metode</span><span>${lastOrder.method}</span></div>
        <div class="rc-row rc-bold" style="margin-top:4px;"><span>TOTAL</span><span>Rp ${lastOrder.total.toLocaleString()}</span></div>
        <div class="rc-dashed"></div>
        <div class="rc-center" style="font-size:10px;color:#64748b;">
            Terimakasih telah berbelanja<br>di ${STORE_NAME}
        </div>`;
}
...
function launchSuccessConfetti() {
    const modal = document.querySelector('#modal-success .modal-pop');
    if (!modal) return;
